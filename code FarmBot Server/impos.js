...
const googleSheetURL =
  "https://script.google.com/macros/s/AKfycbyq81dCnn_y8GefQtuwACzvlXsi9C_KvJyPBT2qYbp0sVxd0_54m388bYUGjRIPEWtd/exec";
...
// (API Endpoints เดิมของคุณจะอยู่ต่อจากนี้)
...
// === ส่วนที่เพิ่มเข้ามาใหม่: Helper Functions สำหรับ POST, PATCH, DELETE ===
...
async function connectMQTT() {
  ...
  // ในฟังก์ชัน connectMQTT()
  mqttClient.on("connect", () => {
    console.log("- MQTT Connected");
    const mqttUser = token.unencoded.bot;

    // Subscribe to status (โค้ดเดิม)
  ...
  mqttClient.on("message", (topic, message) => {
    if (topic.endsWith("/status")) {
      try {
        ...
        sendStatusToAllClients(mqttStatus); // ส่งสถานะใหม่ไปให้ทุก client ทันที
      } catch (e) {
        /* ... */
      }
    }
    if (topic.endsWith("/status")) {
      try {
        ...
        console.warn("⚠️ Invalid MQTT message:", e.message);
      }
    }
  });
...
// === โค้ดส่วน CUSTOM API ENDPOINTS เดิมของคุณ (ไม่ลบ) ===
...
app.post("/api/move_absolute", (req, res) => {
  ...
  res.json({
    message: `Moving to X:${destination.x} Y:${destination.y} Z:${destination.z} at speed ${speed}`,
  });
});
...
app.post("/api/toggle-light", (req, res) => {
  ...
  res.json({ message: `Light set to ${req.body.value === 1 ? "ON" : "OFF"}` });
});
...
app.get("/api/farm_events", async (req, res) => {
  try {
    ...
    res.status(500).json({ error: `Failed to get farm_events: ${e.message}` });
  }
});
...
// === ส่วนที่เพิ่มเข้ามาใหม่: FULL RESTful API ENDPOINTS (ตามเอกสาร) ===
...
// GET /api/sequences/:id is already defined in custom endpoints
...
app.post("/api/ponds", async (req, res) => {
  ...
  try {
    ...
    console.log(`Successfully created point '${name}' with ID: ${newPoint.id}`);
    ...
    for (const templateSeq of templateSequences) {
      ...
      console.log(`Cloning '${templateSeq.name}' to create '${newSeqName}'...`);
    ...
    res.status(201).json({
      message: `Successfully created pond '${name}' and cloned ${createdCount} sequences.`,
      point: newPoint,
    });
  } catch (error) {
    console.error(
...
app.patch("/api/ponds/:pointId", async (req, res) => {
  ...
  try {
    ...
    console.log(
      `[SUCCESS] Point ID ${pointId} ("${pointData.name}") updated successfully.`
    );
    ...
    if (!measureAllSequence || !targetMeasureSequence) {
      console.warn(
        `[WARN] Could not find required sequences for "${MASTER_SEQUENCE_NAME}" update.`
      );
      return res.json({
    ...
    if (firstExecuteIndex === -1) {
      ...
      const header = otherSteps.filter((_, index) => index < firstExecuteIndex);
    ...
    /**
     * THE FINAL FIX:
     * Add the 'name' of the sequence to the payload when patching the sequence.
     */
    ...
    if (measureAllSequence) {
      ...
      if (targetMeasureSequence) {
        ...
        if (measureAllBody.length < initialLength) {
          console.log(
            `[ACTION] Removing "Measure ${pondName}" from "${MASTER_SEQUENCE_NAME}" before deletion.`
          );
...
// 4. API สำหรับแก้ไขและรัน Measure All
app.post("/api/sequences/execute-measure-all", async (req, res) => {
  ...
  if (!selected_pond_names || selected_pond_names.length === 0) {
    return res.status(400).json({ error: "Please select at least one pond." });
  }
  ...
  try {
    ...
    res.json({
      message: `Executing updated "Measure All" sequence for ${newSteps.length} ponds.`,
    });
  } catch (error) {
...
app.post("/api/sequences/weed-pond", async (req, res) => {
  ...
  // === จุดแก้ไขที่ 1: กำหนดเป้าหมายที่จะ "แก้ไข" และ "สั่งรัน" ให้ถูกต้อง ===
  const sequenceNameToPatch = "DELAY WEED MM"; // <--- ซีเควนซ์ที่จะถูกแก้ไขค่า WAIT
  const sequenceNameToRun = `WEED ${pondName}`; // <--- ซีเควนซ์ที่จะถูกสั่งให้ทำงาน
...
app.post("/api/sequences/execute/:id", (req, res) => {
  ...
  try {
    ...
    console.error(
      `Failed to send execution command for sequence ${sequenceId} (Label: ${label}):`,
      error
    );
...
app.get("/api/peripherals", async (req, res) => {
  try {
    ...
    res.status(500).json({ error: "Failed to fetch combined peripheral data" });
  }
});
